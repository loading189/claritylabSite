...
const resource = resources.find((item) => item.slug === 'cash-flow-snapshot');
      ...
      <Container className="max-w-4xl">
        ...
        <Card title="Get access" className="mt-4">
          {runtimeConfig.forms.hasContactForm ? (
            <>
              <p className="mb-4 text-sm text-slate-700">After submitting the form, use the download link below.</p>
          ...
          {runtimeConfig.resources.hasCashflowSnapshot ? (
            <Button href={runtimeConfig.resources.cashflowSnapshotUrl} className="mt-4">
              Get the download
            </Button>
          ) : (
            <p className="mt-4 text-sm text-slate-600">Set NEXT_PUBLIC_RESOURCE_CASHFLOW_URL to enable direct download.</p>
          )}
