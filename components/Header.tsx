...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[rgba(15,17,21,0.75)] backdrop-blur-xl">
      ...
      <div
        id="mobile-nav"
        className={`grid transition-all duration-200 md:hidden ${open ? 'grid-rows-[1fr] border-t border-border bg-bg/95' : 'grid-rows-[0fr]'}`}
      >
          ...
          <Container className="flex flex-col gap-2 py-4">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-button px-3 py-2 text-sm text-muted no-underline hover:text-text"
